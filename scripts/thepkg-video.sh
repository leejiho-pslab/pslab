#!/usr/bin/env bash
# [템플릿] data/<brand>-video.json(segments/output/bgm) 를 읽어 ffmpeg 합성.
# 먼데이투선데이 유튜브 무드필름 합성 (CI 전용, ffmpeg)
# data/thepkg-video.json 의 segments 를 순서대로 정규화(1080x1920@30) →
# 클립엔 자막 오버레이 + 페이드, 카드엔 페이드 → concat → 앰비언트 BGM 믹스.
set -euo pipefail
J=data/thepkg-video.json
OUT=$(jq -r '.output' "$J")
...
# BGM — 커밋된 우아한 라운지 곡($(jq -r '.bgm' "$J"))이 있으면 우선 사용,
# 없으면 저작권 free 합성 패드로 폴백.
fade_out_start=$(echo "$total - 2.2" | bc)
if [ -f $(jq -r '.bgm' "$J") ]; then
  ffmpeg -y -loglevel error -stream_loop -1 -i $(jq -r '.bgm' "$J") -t "$total" \
