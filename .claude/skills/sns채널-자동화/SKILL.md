...
> 🧠 **작업 시작 전 `adr/README.md`(결정 기록 인덱스)를 먼저 확인**한다. 이 스킬은
> **SKILL.md(어떻게=지침) + adr/(왜=결정 이력)** 2계층으로 기억을 유지한다. 중요한 결정·
> 반복 함정을 발견하면 **ADR을 추가**하고 달라진 절차는 SKILL.md에 반영한다(→ §10).
...
- **블로그 개편 규칙** (네이버·구글 **각각 다른 원고**로):
  ...
  - **키워드 기반 기획 시스템(권장·상위호환)**: 상대지수(데이터랩)보다 **실제 월간검색수**가 기획에 정확.
    ...
    - **3시트 구조(v3)**: `core`(핵심=브랜드 최상위 연관)·`related`(연관=파생 관련어)·`timely`(시의성=월별
      시즌 캘린더 시드) 각 검색량 상위 20 + `baseDate`(기준일). 워크플로 **금요일 게이트**로 주1회 갱신(기준일 고정).
    - **차주 주간 스케줄링**: `plan-week.mjs`가 **토요일**에 상위 키워드로 다음주(월~일) 콘텐츠 기획안
      (채널·날짜·제안제목)을 `week-plan.json`에 생성 → 대시보드 "📅 차주 콘텐츠 기획" 패널. **plan.json은
      미변경(안전)** — 실제 제작은 이 기획안 기준으로 감도·가짜글자 검수 후 진행. 시의성 키워드는 주 1회 배치.
...
이 스킬은 두 계층으로 **스스로 기억하고 갱신**한다:

| 문서 | 역할 |
|---|---|
| **이 `SKILL.md`** | **어떻게(how-to)** — 안정된 절차·규약·함정 목록. 계속 다듬는 살아있는 지침. |
| **`adr/` 폴더** | **왜(why)** — 결정마다 맥락·대안·결과를 남긴 이력(마이클 나이가드 형식 + MADR 대안·장단점). |

두 문서는 서로 링크한다. SKILL의 규약 문장은 근거 ADR을 `(관련: ADR-00NN)`로, ADR은 SKILL 절(§)을
가리킨다. **코드·SKILL·ADR을 한 커밋**으로 묶어 근거가 코드와 붙어 다니게 한다.

### 기억·갱신 루프 (작업할 때마다)
1. **읽기** — `adr/README.md` 인덱스를 훑어 지금 건드릴 영역의 ADR을 연다. **되돌리기 어려운
   결정**(브랜드 식별자·발행 정책·브랜치 전략 등)은 그 ADR을 확인하고 **다시 논쟁하지 않는다**
   (뒤집으려면 옛 ADR 상태를 `대체됨`으로 바꾸고 **새 ADR**을 쓴다).
2. **작업**.
3. **쓰기** — 아래 기준에 해당하면 `adr/template.md`를 복사해 **새 ADR 추가**, 달라진 절차는 SKILL에 반영,
   `adr/README.md` 인덱스 표에 한 줄 추가.
4. **커밋** — 코드+SKILL+ADR을 함께 커밋.

### ADR을 남기는 기준
- 되돌리기 어렵거나 비용이 큰 결정 / 직관에 반하는 함정 강제 / 대안이 여럿이었던 선택 /
  나중에 "왜 이렇게 했지?"가 반드시 나올 결정. **사소한 버그픽스·문구수정은 대상 아님.**

> 상세 규약·인덱스는 **`adr/README.md`**. 현재 ADR-0000~0010(시스템 도입, HTML렌더, 크로미움
> 세이프존, 이미지전면형 디자인, 가짜글자 회피, 키워드 3시트, 차주 스케줄링, 템플릿리터럴
> 이스케이프, 채널 스코프, 계정핸들 단일출처, 병렬브랜치 통합)이 기록돼 있다.
