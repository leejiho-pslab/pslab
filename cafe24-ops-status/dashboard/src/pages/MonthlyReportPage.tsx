...
// 표·좋아진/아쉬운은 자동 생성, 전략은 편집용 초안. Word(.docx) 다운로드 제공.
  ...
  return (
    <div className="mr">
      ...
      <div className="mr-head">
        <div>
          <div className="mr-eyebrow">keek · 온라인 광고 월간 리포트</div>
          <h1 className="mr-title">{data.compare_label} → {data.target_label} 성과 비교</h1>
          <div className="mr-sub">매출·ROAS 기준 · 채널별 효율 비교 + 다음달 전략 초안</div>
        </div>
      ...
      <div className="table-wrap">
        <table className="grid-table mr-table">
          <thead><tr><th className="left">채널</th><th>광고비</th><th>노출</th><th>클릭</th><th>CTR</th><th>CPC</th><th>광고매출</th><th>ROAS</th></tr></thead>
          <tbody>
            {data.channel_rows.map((c) => (
              <tr key={c.channel}>
                ...
                <td className="strong">{c.ad_sales}</td>
                <td className="strong">{c.roas}<span className={dcls(c.roas_d)}> {c.roas_d.text}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <p className="mr-auto">※ 전략은 자동 초안입니다. <b>Word로 다운받아 판단에 따라 편집</b>해 보고에 활용하세요.</p>
    </div>
  );
}
