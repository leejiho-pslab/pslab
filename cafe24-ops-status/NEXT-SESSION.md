...
- **네이버SA 파워링크/플레이스 자동 분리**(`ads_naver.py` — `campaignTp` 기준 `naver_powerlink`/`naver_place`/`naver_other` 채널 분리 집계, 추가 자격증명 불필요). `온라인-광고` 스킬에 문서화된 dbrick 프로젝트의 신규 기능 중 1개 반영 완료 — GA4 전환·사이트분석/네이버 키워드 리포트/경쟁사 딥링크·인스타 임베드/지역현황은 **아직 미반영**(사용자 결정·신규 자격증명 필요해 보류).
- 테스트 88개 통과
