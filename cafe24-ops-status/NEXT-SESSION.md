...
- **네이버 키워드 리포트**(광고 히스토리 탭에 Meta 소재/네이버 검색(키워드)/네이버 플레이스 상단 채널탭 추가). 캠페인→광고그룹→키워드 계층을 따라가(`list_adgroups`/`list_keywords`/`fetch_keyword_facts`) 키워드별 노출/클릭/광고비/전환을 `source="keyword"` facts로 저장, `/api/ads/keywords`가 키워드 리포트 테이블(키워드/캠페인/광고비 소진/노출/클릭/CTR/CPC/전환, 정렬 가능)을 제공.
  - `NaverSearchAdClient._match_rows_to_ids`가 `/stats` 응답 행을 요청 id에 매칭(행에 id 필드 있으면 그걸로, 없으면 순서 기반, 길이 안 맞으면 매칭 포기 + 경고 로그)하는 방어 로직으로 구현.
  - **첫 라이브 수집(2026-07-06 02:02 UTC, run 28763092486)에서 "id 수(100)와 응답 행 수(0)가 달라 매칭 불가" 경고가 대량(30회) 발생** — 순차 `/stats` 호출 간격이 0.25~0.3초로 너무 빨라 네이버SA API의 호출 간격 제한(공식 권장 0.5초)에 걸려 빈 응답을 받은 것으로 추정. **조치 완료**: `NaverSearchAdClient`에 `request_interval`(기본 0, `from_account()`에서만 0.5초로 활성화) 기반 중앙 스로틀(`_throttle()`)을 `_get()`에 적용 — 캠페인/광고그룹/키워드/stats 모든 순차 호출이 0.5초 간격을 지키도록 함. 테스트는 인스턴스를 직접 생성해 `request_interval=0`(즉시 실행)이라 영향 없음.
  - **다음 세션에서 반드시 확인**: 스로틀 적용 후 재수집 시 "매칭 불가" 경고가 사라지고 실제 키워드 facts 개수가 충분히 늘어나는지, 대시보드 광고 히스토리 탭의 "네이버 검색(키워드)" 테이블에 실제 키워드가 채워지는지 라이브로 검증.
