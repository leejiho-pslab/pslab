...
- **신규가입수·회원가입율 "—"**: (부분수정) `count_new_customers`가 목록 폴백에서 **일시적 오류(429/5xx)에도 프로세스 전체를 영구 비활성화**하던 버그 수정 → 이제 구조적 오류(403 scope/404·405 경로없음)에만 latch, 일시적 오류는 해당 날짜만 None. 백필 중 한 날 500이 나면 이후 전부 "—"로 빠지던 증상 해소. 여전히 "—"면 회원 API 응답/스코프(`mall.read_customer`) 실제 점검 필요.
