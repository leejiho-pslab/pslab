...
class Cafe24Client:
    ...
    def _is_structural_http_error(exc: httpx.HTTPStatusError) -> bool:
        ...
        403(scope 부족)·404/405(경로없음)·422(요청 자체가 API 계약과 안 맞음.
        예: /customers 목록이 member_id/cellphone 없이는 항상 거부)·501(미구현)은
        몰/토큰/코드 설정이 바뀌기 전까지 계속 실패 → 이후 호출을 생략한다.
        반면 429/5xx/네트워크 오류는 일시적이므로 latch 하지 않는다
        (그 날짜만 None, 다음 날짜는 정상 재시도).
        get()이 401을 이미 한 번 refresh+재시도한 뒤 넘어온 예외이므로, 이 시점의
        401은 "토큰 만료"가 아니라 "재발급해도 권한 없음"을 뜻한다.
        """
        resp = getattr(exc, "response", None)
        code = resp.status_code if resp is not None else 0
        return code in (401, 403, 404, 405, 422, 501)
