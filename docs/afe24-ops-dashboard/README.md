...
- **수집 주기**: **데일리(매일 1회, 전일자 기준)** — 시간별 수집은 하지 않음
- **지표 자유도**: 카페24 지표는 **기존 keek 운영 대시보드와 동일** 구성을 기본값으로 하되, 모두 `config/metrics.yaml`로 정의 → 추가/삭제/순서변경 자유
...
1. **카페24 어드민 대시보드** — 기존 keek 운영 대시보드와 동일 지표 구성 (상세는 §2-1)
...
### 2-1. 카페24 어드민 대시보드 상세 (기존 keek 대시보드 동일 지표)

![카페24 어드민 대시보드 상세](diagrams/05_cafe24_dashboard_detail.png)

| 영역 | 지표 |
|------|------|
| 상단 요약 KPI | 매출 · 광고 매출액 · 주문건수 · 객단가 · 구매전환율 · 매출대비 광고비율 |
| 핵심 지표 기간 비교 | 매출 · 수주건수 · 객단가 · 구매전환율 · 방문자 · 광고비용 · 광고비율 (최근7일/직전7일/당월누적/전월동기/전년동기) |
| 일별 운영 데이터 | 매출·주문 / 방문자 / CRM / 모바일(결제) / PC / 재고·장바구니 / 카테고리 매출 / 베스트 상품 TOP |
| 차트 5종 | 일별 매출추이(누적 vs 목표) · 디바이스별 매출비중 · 신규 vs 재구매 · 일별 광고비 vs 광고비율 · 카테고리 매출 TOP10 |

> 위 지표는 모두 [`config/metrics.example.yaml`](config/metrics.example.yaml)에 선언되어 있어,
> **코드 수정 없이** 카드·표·차트의 지표를 추가/삭제/순서변경/이름변경할 수 있다. 수집은 **데일리(매일 1회)**.
...
**파이프라인**: `트리거(매일 1회 데일리) → 수집 → 정규화 → 집계·저장 → API 제공 → (선택) 알림`.
...
| Phase 1 | 카페24 대시보드 | 자사몰 현황 자동화 + 카페24 Admin API 실연동 | ✅ |
| Phase 2 | 광고 대시보드 | 채널별 ROAS/효율 + 광고매출 비중 | ✅ (집계/화면, API live 연동 대기) |
| Phase 3 | 소재·경쟁사 | 소재 랭킹·피로도, 경쟁사 모니터링 | ✅ (집계/화면, 소스 live 연동 대기) |
| Phase 4 | 자동화 고도화 | 데일리 GitHub Actions + 알림 | ✅ (스케줄러·알림 골격) |

> 구현체: 저장소 루트 [`cafe24-ops-status/`](../../cafe24-ops-status/) — 4탭 React 대시보드 + FastAPI + Python 스킬.
> 현재 데이터는 mock(결정적 샘플), 카페24는 `--mode live` 로 실수집 가능. 광고/소재/경쟁사 live 연동은 각 collector 의 `collect_live` 에 채운다.
