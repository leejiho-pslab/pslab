...
우선 사용**하도록 연결돼 있다. 연동되면 채널별 `conversions`(전환수/전환율/CPA 계산에 쓰임)가
GA4 값으로 자동 대체되고, 광고 탭 하단 **"GA4 사이트 분석"**(방문자·세션·페이지뷰·평균
체류시간)도 함께 수집된다.
연동 후 과거 데이터에도 GA4 값을 적용하려면 `dbrick ads daily collect` 워크플로를
`days: 31` 로 한 번 재실행(백필)하면 된다.
연동 여부는 `/health` 응답의 `ga4_creds`/`ga4_property` 값(true/false)으로 즉시 확인 가능.
