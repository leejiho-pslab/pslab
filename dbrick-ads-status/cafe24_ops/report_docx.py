"""월간 리포트 데이터 → 편집 가능한 Word(.docx) 바이트. (대표님 확정 양식)

양식: 제목/부제/메타 → 한 문장 요약 → 01 한 장 요약표 → 02 좋아진·아쉬운 →
03 매체별 비교표 + 해석 → 04 키워드표 + 코멘트 → 05 다음달 전략(초안).
경쟁사·주석 섹션은 대표님 편집본 기준으로 제외. 전략은 편집용 초안.
"""
from __future__ import annotations

import io

BRICK = (0x9E, 0x4A, 0x34)
INK = (0x20, 0x1C, 0x18)
MUT = (0x6E, 0x65, 0x5B)
POS = (0x2E, 0x7A, 0x57)
NEG = (0xC2, 0x4A, 0x38)
...
def build_report_docx(data: dict) -> bytes:
    ...
    def sec(n, title):
        p = doc.add_paragraph()
        run(p, f"{n}  ", bold=True, color=BRICK, size=13)
    ...
    def table(headers, rows, bold_last=False):
        ...
        for ri, r0 in enumerate(rows):
            ...
            for i, x in enumerate(r0):
                run(cells[i].paragraphs[0], x, bold=(i == 0 or last))

    tl = data["target_label"]; cl = data["compare_label"]

    # 제목
    p = doc.add_paragraph(); run(p, "디브릭 온라인 광고 월간 리포트", bold=True, size=20)
    p = doc.add_paragraph(); run(p, f"{cl} → {tl}  ·  성과 비교 분석 (리드젠/웹문의 기준)", bold=True, color=BRICK, size=12.5)
    p = doc.add_paragraph()
    run(p, "대상  Meta · 네이버 파워링크 · 네이버 플레이스     |     지표 기준  결과(문의)·CPA·CPC·CTR     |     데이터  광고 API 실측",
        color=MUT, size=9)
    p = doc.add_paragraph(); run(p, "한 문장 요약 — ", bold=True, color=BRICK); run(p, data["headline"])
    ...
    # 02
    sec("02", "좋아진 것 · 아쉬운 것")
    p = doc.add_paragraph(); run(p, "◆ 개선된 지표", bold=True, color=POS, size=11.5)
    for s in data["improved"]:
        pp = doc.add_paragraph(style="List Bullet"); run(pp, s)
    p = doc.add_paragraph(); run(p, "◆ 아쉬운 · 점검할 지표", bold=True, color=NEG, size=11.5)
    ...
    for c in data["channel_rows"]:
        crows.append([c["channel"],
                      ...
                      f'{c["conversions"]}',
                      f'{c["cpa"]}'])
    table(["채널", "광고비", "노출", "클릭", "CTR", "CPC", "결과", "CPA"], crows)
    p = doc.add_paragraph(); run(p, "해석 — ", bold=True); run(p, data["interpretation"])
    ...
    if data["top_keywords"]:
        ...
        if data["keyword_note"]:
            p = doc.add_paragraph(); run(p, data["keyword_note"])

    # 05
    sec("05", f"핵심 인사이트 & {_next_month_label(data['target'])} 전략 (초안 · 편집용)")
    for i, s in enumerate(data["strategy_draft"], 1):
        p = doc.add_paragraph(); run(p, f"{i}. ", bold=True, color=BRICK, size=11.5); run(p, s["title"], bold=True, size=11.5)
        pb = doc.add_paragraph(); pb.paragraph_format.left_indent = Cm(0.6); run(pb, s["body"], color=MUT)
